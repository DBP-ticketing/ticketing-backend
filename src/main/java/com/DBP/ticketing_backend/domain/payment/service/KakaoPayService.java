...
public class KakaoPayService {
     ...
     * 카카오페이 결제 준비 (1단계)
     */
    public KakaoPayReadyResponse ready(Long bookingId, Long userId, String itemName, Integer totalAmount) {
        ...
        KakaoPayReadyResponse response = WebClient.create(KAKAO_PAY_READY_URL)
            .post()
     ...
     * 카카오페이 결제 승인 (2단계)
     */
    public KakaoPayApproveResponse approve(String tid, Long bookingId, Long userId, String pgToken) {
        ...
        KakaoPayApproveResponse response = WebClient.create(KAKAO_PAY_APPROVE_URL)
            .post()
            .header(HttpHeaders.AUTHORIZATION, "SECRET_KEY " + secretKey)
            .header(HttpHeaders.CONTENT_TYPE, MediaType.APPLICATION_JSON_VALUE)
            .bodyValue(request)
